...
def yolo_head(feats, anchors, num_classes, input_shape, calc_loss=False):
    ...
    anchors_tensor = tf.cast(anchors, dtype=tf.float32)
    anchors_tensor = tf.reshape(anchors_tensor, [1, 1, 1, num_anchors, 2])

    # Get grid shape from feature map
    grid_shape = tf.shape(feats)
    grid_shape = grid_shape[1:3]
    
    # Create grid
    grid_y = tf.tile(tf.reshape(tf.range(0, grid_shape[0], dtype=tf.float32), [-1, 1, 1, 1]),
        [1, grid_shape[1], 1, 1])
    grid_x = tf.tile(tf.reshape(tf.range(0, grid_shape[1], dtype=tf.float32), [1, -1, 1, 1]),
        [grid_shape[0], 1, 1, 1])
    ...
    grid_shape_tensor = tf.cast(grid_shape, tf.float32)
    box_xy = box_xy / tf.stack([grid_shape_tensor[1], grid_shape_tensor[0]])
    
    # Handle box width and height
    box_wh = tf.exp(feats[..., 2:4])
    box_wh = box_wh * anchors_tensor
    
    # Convert to relative coordinates
    box_wh = box_wh / tf.stack([input_shape[1], input_shape[0]])
...
def yolo_correct_boxes(box_xy, box_wh, input_shape, image_shape):
    ...
    # Convert box coordinates (reverse x,y to y,x)
    box_yx = tf.stack([box_xy[..., 1], box_xy[..., 0]], axis=-1)
    box_hw = tf.stack([box_wh[..., 1], box_wh[..., 0]], axis=-1)
    
    # Calculate new shape and scale
    min_scale = tf.reduce_min(input_shape/image_shape)
    new_shape = tf.round(image_shape * min_scale)
    ...
    image_wh = tf.concat([image_shape, image_shape], axis=-1)
    boxes = boxes * image_wh
    return boxes
...
def yolo_eval(yolo_outputs,
    ...
    input_shape = tf.cast(input_shape[1:3], tf.float32)
    input_shape = input_shape * 32
    ...
    for l in range(num_layers):
        layer_anchors = tf.gather(anchors, anchor_mask[l])
        _boxes, _box_scores = yolo_boxes_and_scores(yolo_outputs[l],
            layer_anchors, num_classes, input_shape, image_shape)
        boxes.append(_boxes)
...
def yolo_loss(args, anchors, num_classes, ignore_thresh=.5, print_loss=False):
    ...
    for l in range(num_layers):
        ...
        raw_true_wh = tf.where(tf.equal(object_mask, 1), raw_true_wh, tf.zeros_like(raw_true_wh))
